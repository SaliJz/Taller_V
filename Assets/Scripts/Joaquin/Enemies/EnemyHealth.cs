...
/// - Parpadeo rápido por cambio de material al recibir daño (configurable).
...
public class EnemyHealth : MonoBehaviour, IDamageable
{
    ...
    #region --- Parpadeo por cambio de material al recibir daño ---
    [Header("Damage Blink (Material Swap)")]
    [Tooltip("Renderer que cambiará sus materiales al recibir daño. Asignar en Inspector.")]
    [SerializeField] private Renderer blinkRenderer;
    [Tooltip("Material 'blanco' (o efecto) que se aplicará temporalmente. Asignar en Inspector.")]
    ...
    public void TakeDamage(float damageAmount, bool isCritical = false)
    {
        ...
        // Iniciar parpadeo por cambio de material al recibir daño
        StartBlinkMaterial();
    ...
    #region --- Implementación parpadeo por material (corregido: no try/catch con yield) ---
    private void StartBlinkMaterial()
    {
        if (blinkRenderer == null || flashMaterial == null) return;

        // si hay rutina en curso, detenerla y restaurar materiales originales antes de empezar de nuevo
        ...
        blinkRoutine = StartCoroutine(BlinkMaterialCoroutine());
    }

    private IEnumerator BlinkMaterialCoroutine()
    {
        if (blinkRenderer == null || flashMaterial == null)
        {
            blinkRoutine = null;
            yield break;
        }

        // guardar copia de materiales originales (por instancia)
        originalMaterials = blinkRenderer.materials;

        int toggles = Mathf.Max(1, blinkCount);
        ...
        try
        {
            for (int i = 0; i < toggles; i++)
            {
                if (!showingFlash)
                {
                    ...
                    blinkRenderer.materials = flashArray;
                    showingFlash = true;
                }
                else
                {
                    // restaurar originales
                    blinkRenderer.materials = originalMaterials;
                    showingFlash = false;
                }
